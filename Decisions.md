...
### D-1.06b-4 · 2026-07-15 · Owner override to merge PR #7 to `main` without the Vercel preview gate
- **Status:** Accepted — owner instruction ("merge it"), **informed** (the owner was shown, in the same exchange, that no preview exists and that the PR ships two tracked launch blockers).
- **Context:** CLAUDE.md §Branch & PR rules make the Vercel preview the sole review gate and say the executing agent never merges its own PR. As in D-1.05.2-6, **no preview was produced** for PR #7 — the Vercel wiring is mid-transition (`belasica` → `belasica-v2`, branch auto-build off). The code was verified locally instead: `npm run build` + `npm run lint` pass on Node 22.23.1; header (sticky masthead, verified "ФК Беласица" wordmark, crest fallback badge on `GET /crest.svg 404`), the 2×2 gallery mosaic (8 photos aligned), and the explore grid were screenshotted in the dev preview, and the four-column footer was confirmed structurally via the DOM.
- **Decision:** On the owner's explicit instruction, merged PR #7 into `main` **without** the preview review. Mirrors the merge-override precedent (D-1.05.2-6, D-1.06-6, D-1.05-3). The executing agent does not merge as a rule — this is a logged, informed owner override.
- **Consequences:** Phase 1.06b ships to `main`. **Both launch blockers remain open and tracked** (placeholder register): the DEMO footer contact/social (D-1.06b-1) and the owed crest asset + favicon (D-1.06b-2). The preview eyeball check stays owed until Vercel is fixed. `NEXT:` is unchanged — reconcile the season/people routes to the live model.
- **Links:** [[D-1.05.2-6]], [[D-1.06b-1]], [[D-1.06b-2]], CLAUDE.md §Branch & PR rules.
